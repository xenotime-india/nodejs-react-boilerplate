 ...
 */
const SalesforceService = (function() {
  ...
  function getOAuth2() {
    return new jsforce.OAuth2({
      ...
      clientId: config.OAUTH_APP_KEY,
      clientSecret: config.OAUTH_APP_SECERET,
      redirectUri: config.OAUTH_APP_CALLBACK
    });
  }
  ...
  function getAuthorizationUrl() {
    return this.getOAuth2().getAuthorizationUrl({scope: config.SF_SCOPE});
  }
  ...
  function processOAuthCode(code) {
    ...
    return self.getConnection()
      ...
      .spread(function(conn, userInfo) {
        const key = `${userInfo.id}${userInfo.organizationId}`;
        const token = JWTService.generateToken(key);
        return [key, token, conn, userInfo, RedisService.del(token)];
      })
      .spread(function(key, token, conn, userInfo) {
        return [token, RedisService.hmset(key, {
